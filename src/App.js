...
function App() {
  ...
  const [resortInfo, setResortInfo] = useState(resorts.length > 0 ? resorts[0] : '')
  ...
  useEffect(() => {
		axios
			...
			.catch((error) => alert(`Unable to find map for ${resortInfo.name}`));
  }, [resortInfo]);
  ...
  const renderSwitch = (page) => {
    switch(page) {
      ...
      case 'ResortInfo':
        return <ResortInfo resort={resortInfo} 
                           viewMap={viewMap} />
    }
  }
